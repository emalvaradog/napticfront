    ...
    <section className={styles.section}>
      <div className={styles.sectionText}>
        ...
        <p>
          ...
          automáticamente y podrás interactuar con tu audio haciéndole preguntas
        </p>
      </div>
      {files.length > 0 && (
        <div className={styles.files}>
          <p>{files.length === 1 && `${files.length} archivo seleccionado:`}</p>
          {files.map((e, index) => (
      ...
      <button className={styles.selectFile} onClick={handleClick}>
        ...
        <p>{files.length > 0 ? "Reemplazar archivo" : "Seleccionar archivo"}</p>
      </button>
      <input
        type="file"
